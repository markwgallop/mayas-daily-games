...
- `renderBuildCharacter(el, doneCount)` — the landing page dragon, assembled one jigsaw
  ...
  animation code does not care what it draws.

  The whole dragon is drawn once into `<defs>` and cut by four `<clipPath>`s. Each interior
  seam is generated by one helper (`_vSeamDown` / `_vSeamUp`, `_hSeamRight` / `_hSeamLeft`)
  and traversed in the opposite direction by the neighbouring piece, so the tabs interlock
  exactly with no gaps. If you change the tab size, change it in `_JIG_T` / `_JIG_B` only.
